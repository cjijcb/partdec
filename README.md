...
Options:
  ...
  -H, -header <HEADER_NAME:VALUE>
    ...
    Can be used multiple times to set or add multiple headers. Range header is ignored
    in multipart web download.
  ...
  -x
    Disable HTTP Keep-Alive or connection reuse. This ensure separate connection
    per output file in multipart web download.
...
Output File States:
    ...
    A file with the [unknown] state is always truncated to 0 size at every run given with
    the same arguments. This state occurs when a web server does not support multipart
    downloading.
```
